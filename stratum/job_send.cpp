...
static void job_mining_notify_buffer(YAAMP_JOB *job, char *buffer)
{
	...
	if (!strcmp(g_stratum_algo, "lbry")) {
		...
		return;
	} else if (g_stratum_usemetronome) {
		//added for BLE
		sprintf(buffer, "{\"id\":null,\"method\":\"mining.notify\",\"params\":[\"%x\",\"%s\",\"%s\",\"%s\",[%s],\"%s\",\"%s\",\"%s\",true,\"%s\"]}\n",
		job->id, templ->prevhash_be, templ->coinb1, templ->coinb2, templ->txmerkles, templ->version, templ->nbits, templ->ntime, templ->metronomehash_be);
		return;
